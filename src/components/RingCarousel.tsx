...
function Carousel<RecordType extends DefaultRecordType>(
	props: CarouselProps<RecordType>
) {
	...
	const carousel = useCallback((index: number) => {
		...
		play(destRotation.current, rotation.current, speed, (data, timer) => {
			...
			const { xOrigin, yOrigin, xRadius, yRadius } = xy(
	...
	return (
		<div
			onClick={(e) => {}}
			ref={boxRef}
			className="w-96 h-96 md:w-[800px]  "
...
export default Carousel as Ring;
